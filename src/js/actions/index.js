...
export const fetchPhotos = createAction(types.SEARCH_FETCHED, (keyword, tag, page) => {
  return axios.get(searchUrlBase + '?term=' + keyword + '&tag=' + tag + '&page=' + page + '&rpp=20&image_size=440&sort=highest_rating&consumer_key=qORoTrkfEDEBtysr6psIO2sKU6aHyvjYj0Aq4dRu')
  .then((res) => {
    if(res) {
      return {
        data: res.data,
